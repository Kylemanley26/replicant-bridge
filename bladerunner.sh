...
echo -e "${YELLOW}Checking background location apps...${NC}"
{
...
if [ -s "$OUTPUT_DIR/background_location_apps.txt" ]; then
    echo -e "${GREEN}✓ Completed${NC}"
else
    echo -e "${GREEN}✓ Completed (no apps found)${NC}"
fi
...
echo -e "${YELLOW}Checking device administrators...${NC}"
{
    ...
    for pkg in $(adb shell pm list packages --user 0 | cut -d: -f2); do
...
echo -e "${YELLOW}Checking SMS interceptors (this may take a while)...${NC}" 
{
    count=0
    for pkg in $(adb shell pm list packages --user 0 | cut -d: -f2); do
        ...
        if [ ! -z "$perms" ]; then
            ...
            if [ $((count % 10)) -eq 0 ]; then
                echo -ne "\r${YELLOW}Progress: Found $count apps with SMS permissions...${NC}" >&2
            fi
        fi
    done
    echo -ne "\r" >&2
...
echo -e "${YELLOW}Checking overlay permission apps...${NC}"
{
    for pkg in $(adb shell pm list packages --user 0 | cut -d: -f2); do
...
echo -e "${YELLOW}Checking boot receivers...${NC}"
{
    for pkg in $(adb shell pm list packages --user 0 | cut -d: -f2); do
...
echo -e "${YELLOW}Checking foreground services...${NC}"
{
...
echo -e "${YELLOW}Checking VPN apps...${NC}"
{
    for pkg in $(adb shell pm list packages --user 0 | cut -d: -f2); do
...
echo -e "${YELLOW}Checking apps with dangerous permission combinations...${NC}"
{
...
run_check "Non-Play Store Apps" \
    "adb shell pm list packages -i --user 0 | grep -v 'installer=com.android.vending' | grep -v 'installer=null' | head -50" \
    "non_playstore_apps.txt"

# Fixed sideloaded apps check
echo -e "${YELLOW}Checking sideloaded apps...${NC}"
{
    adb shell pm list packages -i --user 0 | grep -E "installer=com.android.shell|installer=.*packageinstaller|installer=com.android.packageinstaller"
} > "$OUTPUT_DIR/sideloaded_apps.txt"
if [ -s "$OUTPUT_DIR/sideloaded_apps.txt" ]; then
    echo -e "${GREEN}✓ Completed${NC}"
else
    echo -e "${GREEN}✓ Completed (none found)${NC}"
fi
...
echo -e "${YELLOW}Checking Microsoft apps and services...${NC}"
{
    echo "=== Installed Microsoft Apps ==="
    adb shell pm list packages --user 0 | grep -i microsoft
    
    echo -e "\n=== Microsoft App Permissions ==="
    for pkg in $(adb shell pm list packages --user 0 | grep -i microsoft | cut -d: -f2); do
        echo -e "\n--- $pkg ---"
...

{
    ...
    echo "- Overlay permission apps: $(wc -l < "$OUTPUT_DIR/overlay_apps.txt" 2>/dev/null | tr -d ' ' || echo 0)"
    echo "- Active device admins: $(grep -c "component=" "$OUTPUT_DIR/device_admins.txt" 2>/dev/null || echo 0)"
    echo "- VPN-capable apps: $(wc -l < "$OUTPUT_DIR/vpn_apps.txt" 2>/dev/null | tr -d ' ' || echo 0)"
    echo "- Sideloaded apps: $(wc -l < "$OUTPUT_DIR/sideloaded_apps.txt" 2>/dev/null | tr -d ' ' || echo 0)"
